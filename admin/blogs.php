...
<table class="table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td> <!-- Liste des commentaires associés -->
                    <ul>
                        <?php $comments = getCommentsByArticleId($pdo, $blog['id']); ?>
                        <?php foreach ($comments as $comment) { ?>
                            <li><?= $comment['created_at']; ?> <?= $comment['name']; ?> : <?= $comment['comment']; ?></li>
                            <a href="comment_delete.php?id=<?= $comment['id'] ?>" class="btn btn-outline-danger" onclick="return confirm('Êtes-vous sûr de vouloir supprimer le commentaire ?')">Sup</a>
                        <?php } ?>
                    </ul>
                </td>
            <td>
